...
class _ThixChatPageState extends State<ThixChatPage> with SingleTickerProviderStateMixin {
  ...
  Timer? _clearLastHandledCallTimer;
  ...
  @override
  void dispose() {
    _clearLastHandledCallTimer?.cancel();
  ...
  void _ensureIncomingCallListener(AppUser me) {
    ...
    _incomingCallsSub = _calls.streamIncomingOngoingCalls(receiverId: me.id).listen((calls) {
      ...
      // IGNORER LE MÊME APPEL DÉJÀ TRAITÉ
      if (c.id == _lastHandledCallId) return;
  ...
  Future<void> _showIncomingCallSheet({required AppUser me, required ThixCall call}) async {
    ...
    _clearLastHandledCallTimer?.cancel();
    _clearLastHandledCallTimer = Timer(const Duration(seconds: 5), () {
      if (mounted) _lastHandledCallId = null;
    });
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => _IncomingCallSheet(
        ...
        onAccept: () async {
          ...
          context.pop(); // ferme la feuille entrante
          // ON MAINTIENT _incomingSheetOpen PENDANT TOUT L'APPEL
          _incomingSheetOpen = true;
          await showModalBottomSheet<void>(
            context: context,
            isScrollControlled: true,
          ...
          // APPEL TERMINÉ : ON LIBÈRE
          _incomingSheetOpen = false;
        },
      ),
    );
    // SI ON N'EST PAS DANS LE BRANCHE "onAccept", on relâche le flag
...
class _ThixStatusComposerState extends State<ThixStatusComposer> {
  ...
  setState(() => _busy = true);
  try {
    // ✅ Remplacer FilePicker.pickFiles par FilePicker.platform.pickFiles

...
class _ThixCallSheetState extends State<ThixCallSheet> {
  ...
  Future<void> _initAgora() async {
    ...
    _engine.registerEventHandler(RtcEngineEventHandler(
      ...
      // Signature correcte pour l'audio (v6.3.0)
      onLocalAudioStateChanged: (
        RtcConnection connection,
        LocalAudioStreamState state,
        LocalAudioStreamReason reason,
      ) {
        debugPrint('LocalAudioState: $state, reason: $reason');
        if (state == LocalAudioStreamState.localAudioStreamStateFailed && mounted) {
          _snack('Erreur microphone');
        }
      },
      // Signature correcte pour la vidéo (v6.3.0)
      onLocalVideoStateChanged: (
        VideoSourceType source,
        LocalVideoStreamState state,
        LocalVideoStreamReason reason,
      ) {
        debugPrint('LocalVideoState (source: $source): $state, reason: $reason');
        if (state == LocalVideoStreamState.localVideoStreamStateFailed && mounted) {
          _snack('Erreur caméra');
        }
      },
    ));
  ...
  Future<void> _startCall() async {
    if (widget.isCaller) {
      debugPrint('Call started as caller, waiting for connection');
    }
  }
  ...
  Future<void> _toggleMic() async {
    ...
    _snack(enabled ? 'Micro activé' : 'Micro coupé');
  }
  ...
  Future<void> _toggleCam() async {
    ...
    _snack(enabled ? 'Caméra activée' : 'Caméra coupée');
  }

...
// ✅ Import nécessaire pour les types Realtime
  ...
  Stream<List<Map<String, dynamic>>> streamLatest({String status = 'pending', int limit = 80}) {
    ...
    controller.onListen = () {
      ...
      try {
        ...
        // ✅ Solution 1: Utiliser les types importés avec cast dynamic (simple)
        (channel as dynamic)
            .on(
              'postgres_changes',
              {
            ...
            .subscribe((status, [error]) {
              ...
              if (status == 'CHANNEL_ERROR' || msg.contains('permission') || msg.contains('rls')) {
                startPolling();
              }
              debugPrint('AdminAccessRequestService realtime: status=$status, error=$error');
            });
      } catch (e) {
